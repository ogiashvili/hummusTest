...
function parseSignature(pdfParser, fieldDictionary) {
    console.log(fieldDictionary);
    var value = pdfParser.queryDictionaryObject(fieldDictionary, "V");
    var type = value.getType();
    // if(fieldDictionary.exists('V')){
    //    value = fieldDictionary.queryObject('V');
    // }
    var info = value.toJSObject();
    var ss = value.exists('Contents');

    var newValue = pdfParser.queryDictionaryObject(value, "Contents");
    var bytes = pdfParser.queryDictionaryObject(value, "ByteRange");
    var arrayOfStrings = bytes.toPDFArray().toJSArray();


    var contents = toText(newValue);
...
function parseFieldsValueData(result,pdfParser,fieldDictionary,flags, inheritedProperties) {
    ...
    switch(fieldType) {
        ...
        case 'Sig': {
            ...
            result['value'] = parseSignature(pdfParser, fieldDictionary);
            break;
        }
    }
}
